...
- [ ] T064C [US1] 在 `internal/observability/backend/grafana_smoke_adapter.go` 实现窄 backend evidence adapter；质量门控：使 T064B GREEN，唯一允许解码 T063 `BackendQueryResult`，Tempo/Loki 必须先以 target 的精确 marker attribute 和当前窗口构造查询、再只投影低敏 marker observation/count/error class，禁止 command 或 runner 直接解析 Tempo/Loki/Prometheus 原始 JSON。
