...
- 哪些信息必须可见，哪些信息必须隐藏？
  必须可见的是 runtime mode、Collector 配置是否完整、resource 身份、lifecycle 状态和 exporter failure。必须隐藏的是 secret 原值、API key、token、完整 prompt、用户输入和平台 SDK 内部对象。

## 工程实验

本项目已经把平台接入前置条件拆成可离线验证的小切片：

1. `internal/cmd/observability_bootstrap.go` 解析 noop、local、collector 三种运行模式。默认关闭网络；collector mode 缺配置必须 fail-fast，应用不会直连平台。
...
- 默认配置不会访问真实平台。
- local sink 可用于离线验证。
- collector mode 缺配置时不会外连，并返回启动错误。
...
- **真实平台 opt-in**：应用遥测只显式连接 Collector；独立平台 smoke 或 score adapter 才能按各自边界访问真实后端。
- **窄接口隔离 SDK**：平台 SDK 类型只允许出现在 adapter 或 app-layer；`pkg/ai` 核心不导入平台 SDK。
- **配置解析先挡风险**：collector mode 缺 endpoint、协议或超时时直接失败，而不是让下游调用方自己拼条件或静默 no-op。
...
- **配置半启用**：用户选择 collector mode 却缺少 endpoint、协议或超时时，系统若静默 no-op 会丢失观测事实。
...
3. **Collector 配置不完整**：启动 fail-fast，不打印 secret，也不静默降级。
...
- 哪个字段或边界最容易被误用？
  `enabled`、`mode` 与 signal policy 容易混淆；它们必须只在 bootstrap 边界解释。Collector header 原值也最容易被错误写入配置结果或日志。

- 如果线上出问题，应该先看哪条记录？
  先看解析后的 runtime mode 与 Collector 配置错误，再看 resource 身份和 lifecycle failure status/message，最后查 Collector exporter 的低敏错误。
